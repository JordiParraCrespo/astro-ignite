...
The design is **dark-first**. With no class on `<html>`, the `@theme` values apply. A `.light` class flips the functional tokens to their light values:
...
`ThemeToggle.astro` flips the `.light` class and persists the choice to `localStorage`. An anti-flash inline script in `BaseLayout` applies the stored preference before first paint, so there's no flicker. The starter's anti-flash script falls back to the system preference when nothing is stored.
...
> **Swap the typeface.** Fonts are tokens too. The default is the system font stack (no self-hosted files) — repoint `--font-display` / `--font-mono`, or wire up Astro's `astro:fonts` integration for a custom typeface — see [`FONTS.md`](./FONTS.md).
