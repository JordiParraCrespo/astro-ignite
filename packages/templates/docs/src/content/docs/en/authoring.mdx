...
`<Callout>` highlights important content with a tinted border block. Six
variants cover the common intents — neutral, informational, positive, confirmatory,
cautionary, and destructive.
...
Focus a few lines with `// [!code focus]` — the rest dims until the block is
hovered — and highlight a token with `// [!code word:token]` (or `/token/` in
the fence meta). Two fence flags round it out: `wrap` soft-wraps long lines,
`no-lines` drops the gutter.

```ts
export default defineConfig({
  site: 'https://example.com', // [!code focus]
  i18n: { defaultLocale: 'en', locales: ['en', 'es'] }, // [!code focus]
  integrations: [mdx(), sitemap()],
});
```

```ts /accent/
const tokens = { accent: 'oklch(50% 0.23 270)', border: 'var(--color-border)' };
```

```bash wrap no-lines
npm create astro-ignite@latest my-docs -- --template docs --package-manager pnpm --no-git --no-install
```
...
`<TypeTable type={…}>` renders a props reference — one row per key, with type,
default, and description, plus `required` / `deprecated` markers. Use it on
component reference pages.

<TypeTable
  type={{
    variant: {
      type: "'note' | 'info' | 'tip' | 'check' | 'warning' | 'danger'",
      default: "'note'",
      description: 'Tint, border, and icon of the callout.',
    },
    title: { type: 'string', description: 'Heading text; each variant has a default.' },
    class: { type: 'string', description: 'Extra classes merged onto the root.' },
    dismissible: {
      type: 'boolean',
      default: 'true',
      deprecated: true,
      description: 'Callouts are static; use Banner for dismissable notices.',
    },
  }}
/>
