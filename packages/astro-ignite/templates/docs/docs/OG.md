...
1. The `image` prop passed to `<SEO>` (per-page override, passed down from `<BaseLayout image={...}>`)
2. `siteConfig.defaultOgImage` if it's a plain string
3. `siteConfig.defaultOgImage[locale]` (falling back to the default locale) if it's locale-keyed

The docs template schema has no per-entry `ogImage` field — there is no collection-level override.
