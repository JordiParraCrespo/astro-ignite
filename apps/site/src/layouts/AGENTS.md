...
- `BaseLayout.astro` — the root shell every page wraps. Renders
  ...
  from each page's contributed node, sets `<html lang>`, and emits
...
- Set `<html lang>` from the active locale; route titles/links go
  through i18n (`src/i18n`) and `getRelativeLocaleUrl`. No `dir`
  attribute is set anywhere — there's no RTL support yet.
