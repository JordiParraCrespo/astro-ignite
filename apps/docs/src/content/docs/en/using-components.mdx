...
Every atom's source lives in `src/components/ui/` in your scaffolded project — open any file to see its props and implementation. The originals are in [`packages/registry/base/`](https://github.com/JordiParraCrespo/astro-ignite/tree/main/packages/registry/base) in the monorepo.
