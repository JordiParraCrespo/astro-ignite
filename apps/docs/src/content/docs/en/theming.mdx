...
preference before first paint, so there's no flicker. `siteConfig.defaultTheme`
sets the first-visit default (`dark` for the starter, `light` for docs).
...
This is also the surface the `customize-theme` skill drives — point an agent at
"make the site violet" and it edits these same tokens.
