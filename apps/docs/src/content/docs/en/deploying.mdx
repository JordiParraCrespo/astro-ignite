...
<Callout variant="warning" title="Node ≥ 22.12 on the build machine">
  Both templates run on Astro 7, which requires **Node 22.12 or newer**. Set
  the Node version in your host's build settings (or `.nvmrc` / `engines`)
  before the first deploy — an older default image fails at install time.
</Callout>
...
<Callout variant="warning" title="Docs template: run the package build script">
  The docs template indexes search in a `postbuild` hook —
  `"postbuild": "pagefind --site dist"` in its `package.json`. Configure the
  host's build command as `pnpm build` (or `npm run build`), **not** bare
  `astro build`: the bare binary skips the hook and the site ships without a
  search index.
</Callout>
