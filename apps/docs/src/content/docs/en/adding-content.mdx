...
Frontmatter is `title` (≤70) + `description` (40–160), with optional `tags`,
`lastUpdated`, `draft`, `canonical` (a URL, for republished content), `noindex`
(hide from search engines, `llms.txt`, and `llms-full.txt`), `readingTime` (a
minutes override; auto-computed from word count if omitted), and `landing` (see
below):
...
Reshape the sidebar in `src/config/sidebar.ts`. Each group takes an `icon`,
`collapsed: true` to fold it into a native `<details>` by default, and a `tab`
that sorts groups into top-level sections — the segmented tab row only renders
once two or more distinct tabs exist. Items reference a doc slug, an absolute
path, or an external URL, and can carry their own `icon`; `groupKey` /
`labelKey` point at i18n dictionary keys for translatable labels.

### Landing pages

Set `landing: true` and open the body with the `Hero` component — the layout
then skips its standard title header, drops the on-this-page column, and lets
the content run full width:
...
`Hero` owns the page's `<h1>`; the default slot is the lede and the `actions`
slot a row of buttons.
