...
## What else keys off `siteConfig.locales`

The locales array is the single switch for every locale-scoped surface, not just
the routes:

- **Per-locale RSS.** `/rss.xml` serves the default locale; each non-default
  locale gets `/<lang>/rss.xml` from `src/pages/[lang]/rss.xml.ts`.
- **Markdown routes** _(docs template)_. A doc page gets its `<path>.md` twin
  only when its locale is in `siteConfig.locales` — a translated file in a
  folder that isn't configured renders nothing, in HTML or Markdown.
- **`/llms.txt` and `/llms-full.txt`** _(docs template)_. Both include every
  configured locale (default first); `llms.txt` section headings gain a
  language suffix once more than one locale is active.
