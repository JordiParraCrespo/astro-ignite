...
por honeypot y conectado a un proveedor de correo. En desarrollo registra en la
...
1. El formulario vive en el componente `ContactSection` (`src/components/contact/ContactSection.astro`), compuesto con los átomos del registry `Label`, `Input`, `Select` y `Textarea`, y publica directamente a la acción: `<form method="post" action={actions.contact}>`. Astro maneja la mejora progresiva.
...
<Callout variant="warning" title="Las acciones necesitan un servidor">
...
- **Añadir un campo** → extiende el esquema `input` de Zod en
  `src/actions/index.ts`, luego añade el par `Label` + `Input` (o `Select` /
  `Textarea`) correspondiente en `src/components/contact/ContactSection.astro`,
  y pasa el nuevo valor a `sendContactEmail` en el handler. El tipo fluye
  automáticamente.
- **La protección anti-spam** es solo el honeypot. Cubre bots de bajo esfuerzo;
  un CAPTCHA o desafío (Turnstile, hCaptcha) no se incluye a propósito — añade
  uno si tu bandeja de entrada lo necesita.
