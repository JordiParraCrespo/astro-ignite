...
frontmatter es `title` (≤70) + `description` (40–160), con `tags`, `lastUpdated`,
`draft`, `canonical` (una URL, para contenido republicado), `noindex` (oculta de
los buscadores, de `llms.txt` y de `llms-full.txt`), `readingTime` (minutos, para
sobrescribir el cálculo automático por palabras) y `landing` (mira abajo)
opcionales:
...
Reorganiza la barra lateral en `src/config/sidebar.ts`. Cada grupo acepta un
`icon`, `collapsed: true` para plegarlo por defecto en un `<details>` nativo, y un
`tab` que reparte los grupos en secciones de nivel superior — la fila de pestañas
solo se renderiza cuando existen dos o más pestañas distintas. Los elementos
referencian un slug de doc, una ruta absoluta o una URL externa, y pueden llevar
su propio `icon`; `groupKey` / `labelKey` apuntan a claves del diccionario i18n
para etiquetas traducibles.

### Páginas landing

Pon `landing: true` y abre el cuerpo con el componente `Hero` — el layout entonces
omite su cabecera de título estándar, quita la columna "en esta página" y deja
que el contenido ocupe todo el ancho:

```mdx
---
title: Introduction
description: What this project is, who it's for, and the fastest way to a first successful build.
landing: true
---
...
`Hero` es dueño del `<h1>` de la página; el slot por defecto es la entradilla y
el slot `actions` una fila de botones.
