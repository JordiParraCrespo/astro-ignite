...
<Callout variant="tip" title="Cambia la tipografía">
  Las fuentes también son tokens. El valor por defecto es la pila de fuentes
  del sistema (sin ficheros autoalojados) — reapunta `--font-display` /
  `--font-mono`, o configura la integración `astro:fonts` de Astro para una
  tipografía personalizada — mira el análisis a fondo de FONTS en la carpeta
