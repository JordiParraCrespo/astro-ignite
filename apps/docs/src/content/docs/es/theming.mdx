...
| `--color-accent`, `--color-accent-fg` | Acento de marca — **solo plantilla docs** (enlaces, nav activa, marcador del TOC) |
| `--color-info`, `--color-accent-purple`, `--color-accent-rose` | Acentos de docs de API — **solo plantilla docs** (callouts de info, etiquetas de tipo / ubicación de campos) |
...
<Callout variant="note" title="Acento: starter frente a docs">
  La **starter** es estrictamente monocroma — no tiene token de acento, y el
  color interactivo es simplemente el neutro invertido (casi blanco sobre casi
  negro). La plantilla **docs** entrega un acento coral cálido,
  `--color-accent` (con `--color-accent-fg`), usado como señal y no como
  pintura: enlaces en la prosa, el elemento activo de la barra lateral y de las
  pestañas, el marcador de "en esta página" y el eyebrow del hero. Los botones
  siguen siendo monocromos sobre `--color-primary`. El bloque `.light` reajusta
  el acento (y los acentos info / púrpura / rosa de docs de API) a tonos más
  profundos para que mantengan contraste AA sobre blanco. Introducir un acento
  de marca en la starter es el rediseño más común; el recorrido de abajo hace
  exactamente eso.
</Callout>
...
### La escala de la prosa

El contenido renderizado desde MDX no está en la ruta de escaneo de clases de
Tailwind, así que cada plantilla declara sus reglas de prosa una sola vez,
resueltas por tokens. En la plantilla **docs** viven en un bloque
`@layer components` en `DocsLayout.astro` (`.docs-prose`): cuerpo de 16px /
1.7, `h2` de 24px, `h3` de 19px, enlaces sobre `--color-accent` y los
marcadores de lista restaurados. Al estar en la capa `components` quedan por
debajo de las utilidades, así que un átomo renderizado dentro de la prosa
(Card, Callout, Steps) conserva su propio espaciado. La **starter** declara la
misma idea como un `@utility prose` en `global.css`, de modo que
`class="prose"` se compone con variantes. Reajusta la tipografía ahí, no
página por página.
...
<Callout variant="tip" title="Cambia la tipografía">
  Las fuentes también son tokens. `--font-display` / `--font-mono` nombran
  primero Geist y Geist Mono, pero de serie ningún `@font-face` coincide con
  esos nombres, así que lo que realmente se renderiza es la **pila de fuentes
  del sistema** — cero descargas de fuentes, cero CLS. Para usar una tipografía
  personalizada, añade un `@font-face` autoalojado cuya familia coincida con el
  token (o configura la integración de fuentes de Astro y alinea el token con
  la familia que emite) — mira el análisis a fondo de FONTS en la carpeta
  `docs/` de la plantilla.
</Callout>
...
Como todo pasa por estos tokens, "haz el sitio violeta" es una edición en un
solo archivo — ya lo haga una persona o un agente.
