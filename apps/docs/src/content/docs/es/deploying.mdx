...
<Callout variant="warning" title="Node ≥ 22.12 en la máquina de compilación">
  Ambas plantillas corren sobre Astro 7, que requiere **Node 22.12 o más
  reciente**. Establece la versión de Node en los ajustes de compilación de tu
  host (o en `.nvmrc` / `engines`) antes del primer despliegue — una imagen por
  defecto más antigua falla en la instalación.
</Callout>
...
<Callout variant="warning" title="Plantilla docs: ejecuta el script build del paquete">
  La plantilla docs indexa la búsqueda en un hook `postbuild` —
  `"postbuild": "pagefind --site dist"` en su `package.json`. Configura el
  comando de compilación del host como `pnpm build` (o `npm run build`), **no**
  `astro build` a secas: el binario pelado se salta el hook y el sitio se
  publica sin índice de búsqueda.
</Callout>
...
<Callout variant="warning" title="Un adaptador a la vez">
