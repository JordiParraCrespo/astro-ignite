...
## Qué más depende de `siteConfig.locales`

El array de idiomas es el único interruptor de cada superficie por idioma, no
solo de las rutas:

- **RSS por idioma.** `/rss.xml` sirve el idioma por defecto; cada idioma no
  predeterminado obtiene `/<lang>/rss.xml` desde `src/pages/[lang]/rss.xml.ts`.
- **Rutas Markdown** _(plantilla docs)_. Una página de docs obtiene su gemela
  `<ruta>.md` solo cuando su idioma está en `siteConfig.locales` — un archivo
  traducido en una carpeta no configurada no renderiza nada, ni en HTML ni en
  Markdown.
- **`/llms.txt` y `/llms-full.txt`** _(plantilla docs)_. Ambos incluyen todos
  los idiomas configurados (el predeterminado primero); los encabezados de
  sección de `llms.txt` ganan un sufijo de idioma en cuanto hay más de un idioma
  activo.
