...
>
> `apps/playground/` is the e2e smoke target for `pnpm scaffold:test`. It is fully regenerated by `scripts/scaffold-test.mjs` (which wipes it, runs the CLI with `--yes`, installs, builds, and runs Lighthouse). Any changes you make here will be overwritten on the next CI run.
>
> To fix template output, edit the source in `packages/templates/starter/` instead.
