...
- **Three concerns, three packages.** `packages/astro-ignite/` ships the CLI core. `packages/templates/<kind>/` ships real Astro projects (one per use case: `starter`, `docs`, …). `packages/registry/` ships the component sources + a shadcn-style manifest. The CLI assembles a template; the registry's atoms ship pre-installed in templates that need them. (`packages/create-astro-ignite/` is a thin npm shim that delegates to `astro-ignite` via npx; `packages/design-fetch/` is a build-time tool for the banner pipeline.)
...
```
packages/
 astro-ignite/ # the CLI core (tsup-built, ships as `astro-ignite` bin)
 create-astro-ignite/ # thin npm shim — `npm create astro-ignite` convention
 templates/
 ...
 design-fetch/ # build-time CLI to pull a claude-design bundle for the banner pipeline
apps/
...
pnpm dev:cli # tsup --watch in packages/astro-ignite/ (the CLI core)
pnpm dev:cli:shim # tsup --watch in packages/create-astro-ignite/ (the npm shim)
...
`packages/astro-ignite/src/` is the real CLI. `packages/create-astro-ignite/src/` is a one-file shim that `spawnSync`s `npx astro-ignite bootstrap` and exits.

Real CLI source files:

- `index.ts` — entrypoint; parses the `bootstrap`/`init` subcommand and flags, orchestrates prompts → scaffold → install → git
- `prompts.ts` — the `@clack/prompts` flow (project name, package manager, git init, install, **template**); `TEMPLATE_KINDS` is the source of the template list
- `scaffold.ts` — `ensureEmptyTarget` + `scaffoldProject`: copies the chosen template from `packages/templates/<template>/`, rewrites `package.json` (name + adapter/email dep stripping based on which features the target template actually uses), restores dotfiles (`_gitignore` → `.gitignore`), runs install + git init
...
- `scaffold.test.ts` — vitest contract for the copy + rewrite

**Prepack:** `scripts/copy-templates.mjs` copies `../templates/<kind>/` into `./templates/<kind>/` so the published npm package is self-contained. The `templates/` dir inside `packages/astro-ignite/` is a **generated copy** — never hand-edit it; edit `packages/templates/*` instead.
