...
2. Ensure `~/.npm/_cacache/` is writable — `npx lighthouse` fetches into the cache on first use. `pnpm doctor`'s `npm-cache-writable` check warns when this is missing (e.g. when running under a hardened systemd unit whose `ReadWritePaths=` doesn't cover the npm cache).
