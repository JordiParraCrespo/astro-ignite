...
- On merge, the change directory moves from `openspec/changes/<name>/` to `openspec/archive/<name>/`. When no features are in-flight, `openspec/changes/` does not exist on disk — this is expected.
- Locked practices below are encoded as machine-checked invariants in `openspec/specs/<capability>/spec.md`. Boundary `AGENTS.md` files throughout `packages/`, `apps/`, `scripts/`, and `tests/e2e/` (each symlinked to `CLAUDE.md`) point at the relevant spec.
...
- **Sätteri markdown engine.** Astro 7's default Rust-powered Markdown/MDX processor. Every template's `astro.config.mjs` pins it explicitly — `markdown: { processor: satteri() }` from `@astrojs/markdown-satteri` — so the engine is owned, not implicit, and there's one seam for Sätteri `features` / mdast + hast plugins. GFM, SmartyPants, heading slugs, and directives are native (no remark/rehype plugins — Sätteri doesn't run them, and none are used). The docs template's build-time Shiki transformers still flow through the top-level `markdown.shikiConfig`. RSS bodies are the one exception: `rss.xml.ts` renders them with `markdown-it` (a standalone call, not Astro's pipeline).
