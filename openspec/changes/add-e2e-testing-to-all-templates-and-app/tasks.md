...
- [ ] **T1.** Add `@playwright/test` to the workspace root
...
- [ ] **T2.** Add `tests` to `pnpm-workspace.yaml`'s `packages:` list
...
- [ ] **T3.** Create `tests/e2e/tsconfig.json` that extends the root
      `tsconfig` (or `apps/site`'s) for Astro / Node typings and sets
      `include: ['**/*.ts']`. Confirm `pnpm typecheck` still exits 0.
      Covers **S20**.

- [ ] **T4.** Add a Playwright report / `test-results/` ignore entry
...
- [ ] **T5.** Create `playwright.config.ts` at the workspace root. The
...
- [ ] **T7.** Build `tests/e2e/shared/theme.ts` (S5),
...
- [ ] **T8.** Build `tests/e2e/shared/email.ts` and decide between
...
- [ ] **T9.** Build `tests/e2e/shared/targets.ts` — the canonical
...
- [ ] **T10.** Write the parametrised common specs under
      `tests/e2e/common/`:
...
- [ ] **T11.** Write the starter-specific specs under
      `tests/e2e/starter/`:
      `contact.spec.ts` (S11),
      `blog.spec.ts` (S12).
      Covers **S11**, **S12**.

- [ ] **T12.** Write the docs-template-specific specs under
...
- [ ] **T13.** Write the app-level specs at `tests/e2e/site/` and
...
- [ ] **T14.** Write `tests/e2e/playground/smoke.spec.ts` — homepage + 404 + no console errors, gated behind
...
- [ ] **T18.** Document the workflow: - NEW `tests/e2e/AGENTS.md` — directory layout, how to add a
...
- [ ] **T22.** Add a changeset (`.changeset/add-e2e-testing-to-all-templates-and-app.md`)
