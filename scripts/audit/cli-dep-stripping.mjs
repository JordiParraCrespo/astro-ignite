...
// templates using Actions pin @astrojs/node@^11; no scaffolded output
...
if (checkAdapter) {
  // I2 — every template that uses Astro Actions pins @astrojs/node@^11
  for (const tpl of await templateDirs()) {
    ...
    if (usesActions) {
      ...
      } else if (!/\^?11(\.|$)/.test(adapter)) {
        hits.push({ file: relative(ROOT, pkgPath), line: 0, snippet: `@astrojs/node pinned to ${adapter}; required ^11`, message: 'I2 wrong adapter major' });
      }
    }
  }
}
