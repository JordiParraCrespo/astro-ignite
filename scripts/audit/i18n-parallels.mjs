...
for (const tpl of await templateDirs()) {
  ...
  const pages = await walkFiles(pagesDir, (full, name) => name.endsWith('.astro') || name.endsWith('.mdx'));
  for (const page of pages) {
    const rel = relative(pagesDir, page);
    if (rel.startsWith('[lang]/') || rel.startsWith('[lang]\\')) continue;
    // 404 and trailing dynamic routes don't need parallels by convention
    if (basename(rel) === '404.astro') continue;
    const parallel = join(pagesDir, '[lang]', rel);
    if (!exists(parallel)) {
      hits.push({ file: relative(ROOT, page), line: 0, snippet: `missing parallel at src/pages/[lang]/${rel}`, message: 'parallel missing' });
    }
  }

  if (strict) {
    // I2: spot-check that [lang]/*.astro contains getStaticPaths
    const langPages = await walkFiles(join(pagesDir, '[lang]'), (full, name) => name.endsWith('.astro'));
    for (const page of langPages) {
      const content = await readFile(page, 'utf8');
      if (!/getStaticPaths/.test(content)) {
        hits.push({ file: relative(ROOT, page), line: 0, snippet: 'no getStaticPaths in [lang] page', message: 'missing getStaticPaths' });
      }
    }
  }
      ...
      for (const coll of collections) {
        ...
        if (looseFiles.length > 0) {
          for (const lf of looseFiles) {
            hits.push({ file: relative(ROOT, join(collDir, lf.name)), line: 0, snippet: `loose ${lf.name} in collection root; expected <locale>/<slug>`, message: 'loose content file' });
          }
        }
      }
    }
  ...
  if (checkConfig) {
    ...
    if (exists(siteConfig)) {
      ...
      if (!/locales\s*:\s*\[\s*['"]en['"]\s*\]/.test(content)) {
        hits.push({ file: relative(ROOT, siteConfig), line: 0, snippet: 'siteConfig.locales does not default to ["en"]', message: 'locales default drift' });
      }
    }
  }
}
...
emitResult({
  ...
  pass: hits.length === 0,
  hits,
  notes: hits.length === 0 ? `scanned ${(await templateDirs()).length} template(s)` : `${hits.length} violation(s)`,
});
